...
//        }
        ...
        if (action != null) {
            ...
            Map<String, Integer> map = new HashMap<>();
            for (ProductPojo product : products) {
                int qty = cartDao.getCartItemCount(userName, product.getProductId());
                map.put(product.getProductId(), qty);
            }
            ...
            if (avQty == 0) {
                ...
                status = "Only " + avQty + " no of " + productPojo.getProductName()+ " are available, so we are adding only " + avQty + " products in your cart";
            }
            DemandDao demand = new DemandDaoImpl();
